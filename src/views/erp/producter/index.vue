...
<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" border :data="producterTableList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="厂家ID" prop="producterId" align="center" />
      <el-table-column label="厂家名称" prop="producterName" align="center"/>
      <el-table-column label="厂家编码" prop="producterCode" align="center"/>
      <el-table-column label="联系人" prop="producterPerson" align="center"/>
      <el-table-column label="联系电话" prop="producterTel" align="center"/>
    ...
    >
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="联系电话" prop="producterTel">
          <el-input v-model="form.producterTel" placeholder="请输入联系电话" clearable size="small" />
        </el-form-item>
        <el-form-item label="关键字" prop="keywords">
...
<script>
// 引入生产厂家相关api
import { updateProducter, listProducterForPage, deleteProducterByIds, getProducterById, addProducter } from '@/api/erp/producter/producter'
export default {
  name: 'Producter',
  data() {
    return {
      ...
      queryParams: {
        pageNum: 1, // 默认第一页
        pageSize: 10, // 每页默认10条
        producterName: undefined, // 厂家名称
        keywords: undefined, // 关键字
        producterTel: undefined, // 厂家电话
        status: undefined // 状态
      },
      // 表单数据(添加和修改的模态框中的数据)
      form: {},
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      // 如果row.producterId为undefined，那么就表示点击的是修改按钮，因此要ids的第一个数据就是选中的要修改的数据
      // 然后将该producterId作为查询条件向后台发送请求即可
      // const producterId = row.producterId === undefined ? this.ids[0] : row.producterId
      // 下面这种方式，如果是点击修改按钮得到的数据，那么producterId是一个仅有一个值的数组，传递到后台也可以匹配参数
      // js利用|| 或者 && 简便赋值方式
      ...
      // getDictTypeById(row.producterId).then(res => {
    ...
    // 删除操作(含批量)
    handleDelete(row) {
      // 根据row.producterId是否为undefined来判断是批量删除还是单个删除
      // const producterId = row.producterId === undefined ? this.ids : row.producterId
      // const producterId = row.producterId === undefined ? this.ids[0] : row.producterId
      // 下面这种方式，如果是点击删除按钮得到的数据，那么producterId是一个含有多个值的数组，传递到后台也可以匹配参数
      // js利用|| 或者 && 简便赋值方式
      const producterId = row.producterId || this.ids
      // 确认框显示
      this.$confirm('此操作将永久删除该生产厂家信息, 是否继续?', '提示', {
        ...
        this.loading = true
        // 调用api执行删除操作
        deleteProducterByIds(producterId).then(res => {
          // 关闭遮罩
          this.loading = false
          // 操作成功提示
     ...
     *    个人想法：根据this.form.producterId来区分
     *              如果this.form.producterId不存在那么就是新增，因为producterId会在后台生成
     *               否则为修改操作
     */
    handleSubmit() {
      // 打开遮罩
    ...
    reset() {
      ...
      this.form = {
        ...
        producterLeader: undefined,
        producterNumber: undefined,
        orderNum: 0,
        regNumber: 0,
        status: '0' // 默认选中正常状态
      }
