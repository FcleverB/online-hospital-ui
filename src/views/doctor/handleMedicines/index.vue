  ...
  <div class="app-container" v-loading="loading" :element-loading-text="loadingText">
    ...
    <div v-if="careHistory.registrationId !== undefined">
      ...
      <el-card class="box-card">
        ...
        <el-button type="primary" size="small" icon="el-icon-money" @click="doMedicines">发药</el-button>
      </el-card>
      <!--操作按钮结束-->
      <!--处方和详情信息开始-->
      <el-card class="box-card">
...
export default {
  ...
  methods: {
    ...
    handleQuery() {
      ...
      this.itemObjs = []
      // 调用api执行查询操作
      getChargedCareHistoryOnlyMedicinesByRegistrationId(registrationId).then(res => {
    ...
    doMedicines() {
      ...
      this.$confirm('是否确定发药?', '提示', {
        ...
        doMedicine(itemIds).then(res => {
          ...
          this.resetCurrentParams()
          // 这里应该重新查询一下
          getChargedCareHistoryOnlyMedicinesByRegistrationId(this.queryParams.registrationId).then(res => {
            this.careHistory = res.data.careHistory
            this.careOrders = res.data.careOrders
            this.loading = false
            this.itemObjs = []
